...
由于涉及的技术栈比较杂乱，后端有nodejs和python的数据处理，以及mysql和neo4j两种数据库。

因此最终决定将后端的nodejs和python代码分别装入两个不同的文件夹kg_behind、kg_python中，然后整个项目根目录作为前端环境

### 2.2 运行说明

- 前端：打开项目根目录，直接在控制台输入 `npm run serve`

- 后端：打开 `kg_behind` 文件夹，在控制台输入 `node app.js` 或者 `nodemon app.js(前提是得先安装nodemon)`

- mysql：mysql 先创建好一个数据库，然后在 `kg_behind/db/index.js` 文件下修改连接配置参数

  > 注意：要按我的 kg.sql 文件来创建数据库和表！不然和代码中的字段命名不一致会导致很多报错！

- neo4j：先安装一个桌面版的软件，创建好版本不超过版本5的数据库后，用python进行来连接，进行数据处理和cypher语句的编写

  > neo4j 的配置不管的话，项目也可以运行的，只不过用的是我自己之前处理后保存的 json 数据
  >
  > 如果想用自己的数据，最好就是用 neo4j 来存储数据，然后导出 json 进行各种处理（笨方法就是自己手写 json ，不用 neo4j ）
